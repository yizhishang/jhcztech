...
<form id="qryparm" name="qryparm" action="default.action" method="post">
              ...
              <div class="databox2" style="height: 350px">
                <table width="99%" border="0" cellpadding="0" cellspacing="0" class="tab1">
                  <tr>
                    ...
                    <td class="tdhead">招聘人数</td>
					<td class="tdhead">工作地点</td>
                ...
                <c:forEach var="item" items="${data.page.data}">
		              <tr id="row_<c:out value='${item.jobid}'/>" onDblClick="editFunction('<c:out value="${item.jobid}"/>')" title="双击查看详细内容">
	                    ...
	                    <td>&nbsp;<c:out value="${item.position}" default="--"/>&nbsp;</td>
	                    <td class="td3">&nbsp;<c:out value="${item.wantdept}" default="--"/></td>
	                    <td>&nbsp;<c:out value="${item.amount}" default="--"/>&nbsp;</td>
	                    <td class="td3">&nbsp;<c:out value="${item.workarea}" default="--"/>&nbsp;</td>
	                    <td class="td3">&nbsp;<c:out value="${item.issuedate}" default="--"/>&nbsp;</td>
						<td>&nbsp;<a href="javascript:void(0);" onClick="editFunction('jobid=<c:out value='${item.jobid}'/>')">编辑</a></td>
		              </tr>
       			 </c:forEach>
                </table>
                </div>
