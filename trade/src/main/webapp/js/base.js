...
var BaseObj={
	...
	/*重新登录页面*/
	toLogin:function(){
		window.top.location= "http://localhost:8045/ptwop/login.jsp";
	},
	checkLogin:function(data){//检查是否登录超时
		/*if(data.logoutFlag){
			BaseObj.closeProgress();
			BaseObj.alert('提示',"登录超时,点击确定重新登录.",'error',BaseObj.toLogin);
			return false;
		}*/
		return true;
	},
			 	...
			 	error:function(response, textStatus, errorThrown){
			 		try{
				 		...
				 		if(!BaseObj.checkLogin(data)){
					 		...
					 		BaseObj.alert('提示', data.msg || "请求出现异常,请联系管理员!",'error');
					 	}
			 		}catch(e){
			 			alert(e);
			 			BaseObj.alert('提示',"请求出现异常,请联系管理员!",'error');
			 		}
			 	},
			 	complete:function(){
	...
	submitForm:function(form,callback,dataType){
			var option =
			{
			 ...
			 BaseObj.ajaxSubmit(form,option);
			 
	},
	saveForm:function(form,callback){
		if(form.form('validate')){
			BaseObj.progress('Please waiting','Save ing...');
			//ajax提交form
	...
	getById:function(url,option,callback){
		BaseObj.progress();
		BaseObj.ajaxJson(url,option,function(data){
			BaseObj.closeProgress();
			if(data.success){
				if(callback){
			       	callback(data);
