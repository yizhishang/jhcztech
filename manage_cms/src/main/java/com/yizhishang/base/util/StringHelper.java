...
public final class StringHelper
{
    ...
    {
        boolean b = true;//20140507 modify by  修复对" "为false的bug
        if (str == null) {
            b = true;
        } else {
    ...
    public static boolean isEmail(String str)
    {
        ...
        Pattern pattern = Pattern.compile("^([\\w-\\.]+)@((\\[[0-9]{1,3}\\.[0-9]{1,3}\\.[0-9]{1,3}\\.)|(([\\w-]+\\.)"
                + "+))([a-zA-Z]{2,4}|[0-9]{1,3})(\\]?)$");
    ...
    public static String replace(String template, String placeholder, String replacement, boolean wholeWords)
    {
        ...
        if (loc < 0) {
            ...
            return new StringBuffer(template.substring(0, loc)).append(actualReplacement).append(replace(template
                    .substring(loc + placeholder.length()), placeholder, replacement, wholeWords)).toString();
        }
    }
    ...
    public static String replaceOnce(String template, String placeholder, String replacement)
    {
        ...
        if (loc < 0) {
            ...
            return new StringBuffer(template.substring(0, loc)).append(replacement).append(template.substring(loc +
     ...
     * @param byteCount 字节数（度量标准为中文为两个字节，ASCII字符为一个字节,这样子，刚好匹配ASCII为半角字符，而中文为全角字符，保证在网页上中英文混合的句子长度一致）
     * @return
