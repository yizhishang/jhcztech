package com.yizhishang.plat.template.plan;
...
public class PublishPlan extends Thread
{
    ...
    private void disposeTask()
    {
        ...
        dataList = jdbcTemplateUtil.queryForList("SELECT ID,CATALOG_ID,TYPE,RECURSION,TIME,PUBLISHTIME,MACHINE_ID " +
                "FROM T_PUBLISH_PLAN WHERE MACHINE_ID = ? ORDER BY ID ASC", new Object[] {machineId});
                ...
                if (data != null) {
                    ...
                    //间隔时间发布
                    {
                        ...
                        if (StringHelper.isEmpty(publishTime)) {
                            ...
                            //取当前时间与最后发布时间的毫秒数
                            long compareTime = DateHelper.getDateMiliDispersion(new Date(), DateHelper.parseString
                                    (publishTime));
    ...
    private void publishCatalog(int id, int catalogId, int recursion, String machineId)
    {
        ...
        PublishQueueService publishQueueService = (PublishQueueService) SpringContextHolder.getBean
                ("publishQueueService");
        publishQueueService.add(data);

        JdbcTemplateUtil jdbcTemplateUtil = SpringContextHolder.getBean("jdbcTemplateUtil");
        //发布完成，更新发布时间
        jdbcTemplateUtil.update("UPDATE T_PUBLISH_PLAN SET PUBLISHTIME = ? WHERE ID = ?", new Object[] {DateHelper
                .formatDate(new Date()), new Integer(id)});
    }
