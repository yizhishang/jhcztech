...
public class CatalogDao extends BaseDao
{
    ...
    {
        ArrayList<Object> argList = new ArrayList<Object>();
        String sql = "SELECT CATALOG_ID,PARENT_ID,NAME,CATALOG_NO,ROUTE,LINK_URL,CHILDRENNUM FROM T_CATALOG WHERE " +
                "STATE = 1 AND ROUTE like ? AND SITENO = ? ORDER BY ORDERLINE";
        if (catalogId == 1) {
            argList.add("1|%");
        } else {
            argList.add("%|" + catalogId + "|%");
