...
public class LogDao extends BaseDao
{
    ...
    {
        String sql = "DELETE FROM T_LOG WHERE CREATE_BY = (SELECT UID2 FROM T_USER WHERE ID = ?) AND CREATE_DATE > ? " +
                "" + "AND DESCRIPTION LIKE '%密码错误%'";
        getJdbcTemplateUtil().update(sql, new Object[] {new Integer(userId), DateHelper.formatDate(new Date(),
                DateHelper.pattern_date)});
    }
    ...
    {
        String sql = "SELECT COUNT(ID) FROM T_LOG WHERE CREATE_BY = ? AND CREATE_DATE > ? AND DESCRIPTION LIKE " +
                "'%密码错误%'";
        return getJdbcTemplateUtil().queryInt(sql, new Object[] {loginId, DateHelper.formatDate(new Date(),
                DateHelper.pattern_date)});
    ...
    {
        //String sql = "SELECT TRUNC(SYSDATE - TO_DATE(CREATE_DATE,'yyyy-MM-dd HH24:mi:ss')) FROM T_LOG WHERE
        // CREATE_BY = ? AND DESCRIPTION LIKE '修改密码成功%' ORDER BY CREATE_DATE DESC";
        String sql = "SELECT CREATE_DATE FROM T_LOG WHERE CREATE_BY = ? AND DESCRIPTION LIKE '修改密码成功%' ORDER BY " +
                "CREATE_DATE DESC";
