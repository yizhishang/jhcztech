...
public class ArticleDao extends BaseDao
{
    ...
    public List<DynaModel> findPublishArticleById(int catalogId, int rows, int sortType)
    {
        try {
            ...
            StringBuffer sql = new StringBuffer("SELECT T1.CATALOG_ID, T1.ARTICLE_ID,T1.TITLE,T1.BRIEF,T1" +
                    ".PUBLISH_DATE,T1.URL, T1.SOURCE,T2.CONTENT FROM T_ARTICLE T1 LEFT JOIN T_ARTICLE_CONTENT T2 ON " +
                    "T1.ARTICLE_ID = T2.ARTICLE_ID WHERE T1.STATE = 3");
            if (catalogId > 0) {
    ...
    public List<DynaModel> findUnionArtilceByCatalog(int catalogId)
    {
        try {
            List<String> argList = new ArrayList<String>();
            String sql = "SELECT C.CATALOG_ID FROM T_ARTICLE A,T_CATALOG C WHERE A.CATALOG_ID = C.CATALOG_ID AND C" +
                    ".ROUTE LIKE ? GROUP BY C.CATALOG_ID";
    ...
    public DBPage<DynaModel> getPageData(int curPage, int numPerPage, String siteNo, int catalogId, String title, int
            state, String author, String startDate, String endDate)
    {
    ...
    public DBPage<DynaModel> getPageDataByEmail(int curPage, int numPerPage, String siteNo, int catalogId, String
            title, int state, String author, String branchno)
    {
        DBPage<DynaModel> page = null;

        StringBuffer sqlBuffer = new StringBuffer();
        ArrayList<Object> argList = new ArrayList<Object>();
        sqlBuffer.append("select a.*,c.branchname from T_ARTICLE A,T_USER B,T_B_BRANCH C WHERE A.CREATE_BY = B.UID2 "
        ...
        try {
            ...
            if (dataRow.containsKey("content")) {
                ...
                if (isArticleContent(dataRow.getInt("article_id"))) {
                    getJdbcTemplateUtil().update("T_ARTICLE_CONTENT", contentRow, "article_id", new Integer(article
                            .getId()));
                } else {
    ...
    {
        String sql = "SELECT * FROM (SELECT ARTICLE_ID, LEAD(ARTICLE_ID, 1, 0) OVER(ORDER BY PUBLISH_DATE DESC) AS " +
                "DOWN_ID, LAG(ARTICLE_ID, 1, 0) OVER(ORDER BY PUBLISH_DATE DESC) AS UP_ID FROM T_ARTICLE WHERE " +
                "CATALOG_ID = ? AND STATE = 3) WHERE ARTICLE_ID = ?";
        try {
