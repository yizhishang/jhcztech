...
public class EnumAction extends BaseAction
{
    ...
    Result addItem()
    {
        ...
        try {
            ...
            if (StringHelper.isEmpty(enum_type) || StringHelper.isEmpty(item_name) || StringHelper.isEmpty
                    (item_value)) {
                throw new Exception();
            }
        } catch (Exception e) {
    ...
    String doShowTree(HttpServletResponse response)
    {
        ...
        for (Iterator<DynaModel> iter = enumList.iterator(); iter.hasNext(); ) {
            ...
            buffer.append("   <tree text=\"" + dataRow.getString("enum_name") + "\" target=\"roleRightFrame\" " +
                    "action=\"list.action?type=" + dataRow.getString("enum_value") + "\" value=\"" + dataRow
                    .getString("enum_value") + "\" src=\"\"  oncontextmenu=\"true\" />\n");
        }
