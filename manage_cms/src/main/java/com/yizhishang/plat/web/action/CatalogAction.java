...
public class CatalogAction extends BaseAction
{
    ...
    public void add(HttpServletResponse response)
    {
        ...
        //更新该栏目中的父栏目拥有的节点数
        catalogService.updateParentCatalogNum(catalog.getParentId(), catalogService.findParentCatalogNum(catalog
                .getParentId()));
        ...
        if (catalogId > 0 && parentId > 0) {
            ...
            if (parentCatalog != null && catalogs != null) {
                ...
                for (Iterator<Catalog> iter = catalogs.iterator(); iter.hasNext(); ) {
                    ...
                    if (startIndex > 0) {
                        buffer.append(catalog.getId()).append(":").append(parentRouteStr).append("|").append(route
    ...
    public void doDelete(HttpServletResponse response)
    {
        ...
        if (catalogService.hasChildren(catalogId)) {
            ...
            if (catalog.getIsSystem() == 1) //系统目录
            {
                ...
                catalogService.deleteCatalog(catalogId);
                //更新该栏目中的父栏目拥有的节点数
                catalogService.updateParentCatalogNum(catalog.getParentId(), catalogService.findParentCatalogNum
                        (catalog.getParentId()));
    ...
    public String doShowChild(HttpServletResponse response)
    {
        ...
        if (catalogId <= 0) //显示根目录
        {
            ...
            buffer.append("   <tree text=\"" + rootCatalog.getName() + "\" target=\"catalogRightFrame\" " +
                    "action=\"doEdit.action?catalogId=" + rootCatalog.getId() + "\" src=\"showChild" +
                    ".action?catalogId=" + rootCatalog.getId() + "\"  value=\"" + rootCatalog.getId() + "\" " +
                    "oncontextmenu=\"true\" />\n");
        } else {
            ...
            for (Iterator<Catalog> iter = childrenList.iterator(); iter.hasNext(); ) {
                ...
                buffer.append("   <tree text=\"" + name + "\" target=\"catalogRightFrame\" action=\"doEdit" +
                        ".action?catalogId=" + catalog.getId() + "\"  src=\"" + src + "\"  value=\"" + catalog.getId
                        () + "\" oncontextmenu=\"true\" />\n");
            }
        }
    ...
    public String getRoleFunction(DynaModel data, HashSet<String> attachMap)
    {
        ...
        buffer.append("<input type=\"checkbox\"  name=\"id_fun\" id=\"node_" + data.getString("route").replaceAll
                ("\\|", "_") + "\" value=\"" + catalogId + "\"  " + compareRole(catalogId, attachMap) + "/><label>" +
                data.getString("name") + "</label>");
        return buffer.toString();
    }
