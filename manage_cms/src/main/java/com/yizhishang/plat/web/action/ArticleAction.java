...
public class ArticleAction extends BaseAction
{
    ...
    public Result add(HttpServletRequest request, HttpServletResponse reponse)
    {
        ...
        if (articleId > 0 && StringHelper.isNotEmpty(attach_url) && StringHelper.isNotEmpty(attach_savename) &&
                StringHelper.isNotEmpty(attach_realname)) {
        ...
        if (chooseCopyCatalogs != null && chooseCopyCatalogs.length > 0) {
            ...
            for (int i = 0; i < chooseCopyCatalogs.length; i++) {
                ...
                article.setCatalogNo(catalogService.findCatalogById(Integer.parseInt(chooseCopyCatalogs[i]))
                        .getCatalogNo());
    ...
    public ModelAndView doAjaxReadCatalogTree()
    {
        ...
        @SuppressWarnings(
                "rawtypes") List<Map> dataList = catalogService.findCatalogTrue(0, siteNo, SysLibrary.isSystemAdmin
                (getSession()), catalogRole);
    ...
    public String doAjaxShowCatalogRoleRight()
    {
        ...
        String html = treeManage.getChildrenTreeHtml(childrenCatalogs, SysLibrary.isSystemAdmin(getSession()),
                catalogRole);
    ...
    public String doAjaxShowChildsCatalogsToCT()
    {
        ...
        for (Iterator<Catalog> iter = childrenList.iterator(); iter.hasNext(); ) {
            ...
            {
                String url = getRequest().getContextPath() + "/admin/CGWSArticle" +
                        ".action?function=AjaxShowChildsCatalogsToCT&catalogId=" + catalog.getId();
        ...
        if (isPostBack()) {
            ...
            for (int i = 0; i < idArray.length; i++) {
                ...
                for (int j = 0; j < arrCatalogs.length; j++) {
                    ...
                    article.setCatalogNo(catalogService.findCatalogById(Integer.parseInt(arrCatalogs[j]))
                            .getCatalogNo());
                    article.setPublishBy(getUID());
                    article.setCreateDate(DateHelper.formatDate(new Date()));
                    if (1 == isPublish) {
    ...
    public ModelAndView doList()
    {
        ...
        DBPage<DynaModel> page = articleService.getPageData(curPage, SysConfig.getRowOfPage(), siteno, catalogId,
                title, state, author, startDate, endDate);
        dataMap.put("page", page);
    ...
    public String doMessage()
    {
        ...
        if ("virtualCatalog".equals(type))//虚拟栏目
        {
            ...
            dataMap.put("message",
                    "对不起，该栏目属于虚拟栏目只读类型，请到<a href='article.action?function=list&menuNo=&catalogId=" + catalogId + "'>源栏目</a>（" + parentCatalogs + "）中管理文章！");
            return ERROR;
        }
    ...
    public String doShowCatalog(HttpServletRequest request, HttpServletResponse response)
    {
        ...
        if (catalogId <= 0) //显示根目录
        {
            ...
            buffer.append("   <tree text=\"" + rootCatalog.getName() + "\" target=\"articleRightFrame\" " +
                    "action=\"javascript:void(0);\" src=\"showCatalog.action?root&amp;menuNo=" + menuNo + "&amp;" +
                    "catalogId=" + rootCatalog.getId() + "\"  value=\"" + rootCatalog.getId() + "\" " +
                    "oncontextmenu=\"true\" />\n");
        } else {
            ...
            for (Iterator<Catalog> iter = childrenList.iterator(); iter.hasNext(); ) {
                ...
                buffer.append("   <tree text=\"" + name + "\" target=\"articleRightFrame\" action=\"" + url + "\" " +
                        "src=\"" + src + "\" value=\"" + catalog.getId() + "\" oncontextmenu=\"true\" />\n");
            }
        }
    ...
    public Result edit(HttpServletRequest request, HttpServletResponse reponse)
    {
        ...
        if (chooseCopyCatalogs != null && chooseCopyCatalogs.length > 0) {
            ...
            for (int i = 0; i < chooseCopyCatalogs.length; i++) {
                ...
                copyArticle.setCatalogNo(catalogService.findCatalogById(Integer.parseInt(chooseCopyCatalogs[i]))
                        .getCatalogNo());
            }
        }
