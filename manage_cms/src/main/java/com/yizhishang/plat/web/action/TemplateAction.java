...
public class TemplateAction extends BaseAction
{
    ...
    public ModelAndView doList()
    {
        ...
        DBPage<DynaModel> page = templateService.findTemplate(curPage, SysConfig.getRowOfPage(), catalogId, keyword,
                templateState, startDate, endDate, getSiteNo());
    ...
    public void doShowChild(HttpServletRequest request, HttpServletResponse response)
    {
        ...
        if (catalogId <= 0) //显示根目录
        {
            ...
            buffer.append("   <tree text=\"" + rootCatalog.getName() + "\" target=\"catalogRightFrame\" action=\"" +
                    url + "\" src=\"doShowChild.action?root&amp;menuNo=" + menuNo + "&amp;catalogId=" + rootCatalog
                    .getId() + "\"  value=\"" + rootCatalog.getId() + "\" oncontextmenu=\"true\" />\n");
        } else {
            //查询子栏目
            ...
            for (Iterator<Catalog> iter = childrenList.iterator(); iter.hasNext(); ) {
                ...
                buffer.append("   <tree text=\"" + name + "\" target=\"catalogRightFrame\" action=\"" + url + "\" " +
