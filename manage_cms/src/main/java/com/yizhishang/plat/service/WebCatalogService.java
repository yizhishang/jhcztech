...
public class WebCatalogService extends BaseService
{
    ...
    {
        String sql = "SELECT CATALOG_ID,NAME,CATALOG_NO, SITENO,LINK_URL,PARENT_ID,USER_RIGHT FROM T_CATALOG WHERE "
                + "CATALOG_ID=? AND STATE=1 AND SITENO=? order  BY ORDERLINE";
        try {
            return getJdbcTemplateUtil().queryMap(sql, new Object[] {new Integer(ConvertHelper.strToInt(catalogId)),
                    Constants.MAIN_SITENO});
    ...
    {
        String sql = "SELECT CATALOG_ID,NAME,CATALOG_NO, SITENO,LINK_URL,PARENT_ID,USER_RIGHT FROM T_CATALOG WHERE "
                + "CATALOG_ID=? AND STATE=1 AND CATALOG_NO=? order  BY ORDERLINE";
        try {
            return getJdbcTemplateUtil().queryMap(sql, new Object[] {new Integer(ConvertHelper.strToInt(catalogNo)),
                    Constants.MAIN_SITENO});
        } catch (SQLException e) {
            logger.error(e.getMessage());
            return null;
    ...
    {
        String sql = "SELECT CATALOG_ID,NAME,CATALOG_NO,ROUTE,LINK_URL,CHILDRENNUM FROM T_CATALOG WHERE STATE = 1 " +
                "AND" + " ROUTE like ? ORDER BY ORDERLINE";
