...
public class TemplateService extends BaseService
{
    ...
    public void editTemplateByList(List<DynaModel> dataList) throws Exception
    {
        try {
            ...
            for (Iterator<DynaModel> iter = dataList.iterator(); iter.hasNext(); ) {
                ...
                if (this.getJdbcTemplateUtil().queryInt("SELECT ID FROM T_TEMPLATE WHERE ID = ?", new Object[] {new
                        Integer(id)}) == 0) {
    ...
    public DBPage<DynaModel> findTemplate(int curPage, int numPerPage, int catalogId, String keyword, int state,
                                          String startDate, String endDate, String siteNo)
    {
    ...
    {
        ArrayList<Object> argList = new ArrayList<Object>();
        String sql = "SELECT * FROM T_TEMPLATE WHERE CATALOG_ID IN (SELECT CATALOG_ID FROM T_CATALOG WHERE ROUTE " +
                "LIKE" + " ? AND STATE = 1) ORDER BY ID ASC";
        argList.add("%" + route + "%");
        List<DynaModel> dataList = getJdbcTemplateUtil().queryForList(sql, argList.toArray());
