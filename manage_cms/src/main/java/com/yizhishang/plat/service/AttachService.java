...
public class AttachService extends BaseService
{
    ...
    public void addAttach(DynaModel dataRow)
    {
        if (dataRow != null) {
            ...
            if (save_filename_list.length > 0 && save_filename_list.length == real_filaname_list.length &&
                    save_filename_list.length == url_list.length) {
    ...
    public void editAttach(int source, int sourceId, DynaModel dataRow)
    {
        if (dataRow != null) {
            ...
            String[] existsFile = getJdbcTemplateUtil().queryStringArray(sql, new Object[] {new Integer(source), new
                    Integer(sourceId)});
            ...
            if (addAttachList != null && addAttachList.size() > 0) {
                ...
                if (StringHelper.isNotEmpty(save_filename) && StringHelper.isNotEmpty(real_filaname) && StringHelper
                        .isNotEmpty(url)) {
