...
public class PrintValueTag extends CommonTag
{
    ...
    public int doEndTag()
    {
        ...
        if (!"Y".equals(is_pk)) {
            if (input_type.equals(Consts.input_type_checkbox) || input_type.equals(Consts.input_type_radio) ||
                    input_type.equals(Consts.input_type_select)) {
                ...
                if (reuslt.lastIndexOf(".") > 0) {
                    ...
                    if (pics.indexOf(fix.toLowerCase()) != -1) { // 是图片,展示略缩图
    ...
    public String getOptionText(String colvalue, String input_type)
    {
        ...
        if (!"".equals(select_table) && !"".equals(select_value)) {
            if (!Consts.input_type_checkbox.equals(input_type)) {
                String sql = "select distinct " + select_text + " as optiontext from " + select_table + " where " +
                        select_value + "=? " + select_condition;
                ArrayList<Object> argList = new ArrayList<Object>();
                argList.add(colvalue);
                BaseService bservice = new BaseService();
                List<DynaModel> list = bservice.getJdbcTemplateUtil().queryForList(sql, DynaModel.class, argList
                        .toArray());
                ...
                String sql = "select distinct " + select_text + " as optiontext from " + select_table + " where " +
                        select_value + " in (" + colvalue + ") " + select_condition;
                BaseService bservice = new BaseService();
