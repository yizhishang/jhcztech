...
var sMenu1="<TABLE border=0 cellpadding=0 cellspacing=0 class=Menu width="+lang["UIMenuWidth"]+" ondragstart=\"return false;\" onselectstart=\"return false;\" oncontextmenu=\"return false;\"><tr><td width=18 valign=bottom align=center style='background:url(sysimage/contextmenu.gif);background-position:bottom;'><\/td><td width="+(lang["UIMenuWidth"]-18)+" class=RightBg><TABLE border=0 cellpadding=0 cellspacing=0>";
...
function showContextMenu(event){
	...
	// By Guidy 2005-11-05
	sMenu += sMenuHr;
	sMenu += getMenuRow("", "goStudio()", "createlink.gif", lang["goStudioText"]);
	height += 22;
...
function showToolMenu(menu){
	...
	case "file":
		...
		sMenu += sMenuHr;
		sMenu += getMenuRow("", "sizeChange(65)", "sizeplus.gif", lang["SizePlus"]);
		sMenu += getMenuRow("", "sizeChange(-65)", "sizeminus.gif", lang["SizeMinus"]);
		sMenu += sMenuHr;
		sMenu += getMenuRow("", "format('Print')", "print.gif", lang["Print"]);
		sMenu += sMenuHr;
		sMenu += getMenuRow("", "showDialog('about.htm')", "about.gif", lang["About"]);
		sMenu += getMenuRow("", "window.open('http://www.ewebeditor.com')", "site.gif", lang["Site"]);
		height = 208;
		break;
	case "table":
