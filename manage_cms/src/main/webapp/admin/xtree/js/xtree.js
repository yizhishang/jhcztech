...
function WebFXTreeAbstractNode(sText, oAction,sTarget,sValue,sRadio,sCheckbox,sChecked,sDisabled,sOncontextmenu,sColor) {
	this.childNodes = [];
	...
	if (sValue)  this.value  = sValue;
	if (sRadio) this.radio = sRadio;
	if (sCheckbox) this.checkbox = sCheckbox;
	if (sChecked) this.checked = sChecked;
	if (sDisabled) this.disabled = sDisabled;
	if (sOncontextmenu) this.oncontextmenu = sOncontextmenu;
	if (sColor) this.color = sColor;
...
//配置相应的参数
_p.target = webFXTreeConfig.defaultTarget;
_p.value = webFXTreeConfig.defaultValue;
_p.radio = webFXTreeConfig.defaultRadio;
_p.checkbox = webFXTreeConfig.defaultCheckbox;
_p.checked = webFXTreeConfig.defaultChecked;
_p.disabled = webFXTreeConfig.defaultDisabled;
_p.oncontextmenu = webFXTreeConfig.defaultOncontextmenu;
_p.color = webFXTreeConfig.defaultColor;
...
_p.getLabelHtml = function getLabelHtml() {
	...
	var target = this.getTarget();
	if (this._getHref()==""){
	return "<a  class=\"webfx-tree-item-label\" tabindex=\"-1\"" +
		(toolTip ? " title=\"" + webFXTreeHandler.textToHtml(toolTip) + "\"" : "") +
		(target ? " target=\"" + target + "\"" : "") +
		" onfocus=\"webFXTreeHandler.handleEvent(event)\"" +
...
//添加的方法
_p.getoncontextmenuHtml = function getoncontextmenuHtml()
{
  if (this.oncontextmenu=="true")
  {
    // return "oncontextmenu = showContextMenu('"+this.value+"')";
     return "oncontextmenu = showContextMenu()";
   }
   return "";
}

_p.getcheckboxHtml = function getcheckboxHtml()
{
    if (this.value == "")
        return "";
    ...
    if (this.checked == "true")
    {
        ...
        {
            return "<input type=\"checkbox\"   name=\"selectedValue\"  value=\"" + this.value + "\" checked disabled   >";
        }
        else
        {
            return "<input type=\"checkbox\"   name=\"selectedValue\"  value=\"" + this.value + "\" checked    >";
        }
    }
    else
    {
        if (this.disabled == "true")
        {
            return "<input type=\"checkbox\"   name=\"selectedValue\" value=\"" + this.value + "\" disabled >";
        }
        else
        {
            return "<input type=\"checkbox\"   name=\"selectedValue\" value=\"" + this.value + "\"  >";
        }
    }
};


_p.getradioHtml = function getradioHtml()
{
    if (this.value == "")
        return "";
    ...
    if (this.checked == "true")
    {
        if (this.disabled == "true")
        {
            return "<input type=\"radio\"   name=\"selectedValue\" value=\"" + this.value + "\" checked disabled   >";
        }
        else
        {
            return "<input type=\"radio\"   name=\"selectedValue\" value=\"" + this.value + "\" checked    >";
        }
    }
    else
    {
        ...
        if (this.disabled == "true")
        {
            return "<input type=\"radio\"   name=\"selectedValue\" value=\"" + this.value + "\" disabled >";
        }
        else
        {
            return "<input type=\"radio\"   name=\"selectedValue\" value=\"" + this.value + "\"  >";
        }
    }
};
...
_p.getIconHtml = function getIconHtml() {
	...
	// HTML...
	return "<img class=\"webfx-tree-icon\" src=\"" + this.getIconSrc() + "\"  onclick=\"webFXTreeHandler.toggle(this);\">";
};
*/
...
_p.getIconHtml = function getIconHtml()
{
   return "<img class=\"webfx-tree-icon\" src=\"" + this.getIconSrc() + "\"  onclick=\"return webFXTreeHandler.handleEvent(event)\" onmousedown=\"return webFXTreeHandler.handleEvent(event)\" ondblclick=\"return webFXTreeHandler.handleEvent(event)\" onkeydown=\"return webFXTreeHandler.handleEvent(event)\" onkeypress=\"return webFXTreeHandler.handleEvent(event)\">";
};
...
_p.getExpandIconHtml = function getExpandIconHtml() {
	// here we are not using textToHtml since the file names rarerly contains
	// HTML...
	return "<img class=\"webfx-tree-expand-icon\" src=\"" + this.getExpandIconSrc() + "\" onclick=\"webFXTreeHandler.toggle(this);\">";
};
*/

//修改后的方法
_p.getExpandIconHtml = function getExpandIconHtml() {
	return "<img class=\"webfx-tree-expand-icon\" src=\"" + this.getExpandIconSrc() + "\"  onclick=\"return webFXTreeHandler.handleEvent(event)\" onmousedown=\"return webFXTreeHandler.handleEvent(event)\" ondblclick=\"return webFXTreeHandler.handleEvent(event)\" onkeydown=\"return webFXTreeHandler.handleEvent(event)\" onkeypress=\"return webFXTreeHandler.handleEvent(event)\">";
};
...
{
    if(this.color)
	  return "<font color=\""+this.color+"\">"+this.text+"<font>";
	else
	  return this.text;
};
...
function WebFXTreeItem(sText, oAction, eParent, sIcon, sOpenIcon,sTarget,sValue,sRadio,sCheckbox,sChecked,sDisabled,sOncontextmenu,sColor) {
	WebFXTreeAbstractNode.call(this, sText, oAction);
	if (sIcon) this.icon = sIcon;
	if (sOpenIcon) this.openIcon = sOpenIcon;
