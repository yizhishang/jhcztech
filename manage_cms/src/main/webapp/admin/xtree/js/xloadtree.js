...
WebFXLoadTree._attrs = ["text", "src", "action", "id", "target","value","radio","checkbox","checked","disabled","oncontextmenu","color"];
...
WebFXLoadTree.createItemFromElement = function (oNode) {
  ...
  if(jsAttrs.value)
  {
  	jsNode.value = jsAttrs.value;
  }
  
  if(jsAttrs.radio)
  {
  	jsNode.radio = jsAttrs.radio;
  }
  
  if(jsAttrs.checkbox)
  {
  	jsNode.checkbox = jsAttrs.checkbox;
  }
  
  if(jsAttrs.checked)
  {
    jsNode.checked = jsAttrs.checked;	
  }
  
  if(jsAttrs.disabled)
  {
  	jsNode.disabled = jsAttrs.disabled;
  }
  
  if(jsAttrs.oncontextmenu)
  {
  	jsNode.oncontextmenu = jsAttrs.oncontextmenu;
  }
  
  if(jsAttrs.color)
  {
  	jsNode.color = jsAttrs.color;
  }
...
WebFXLoadTree.documentLoaded = function (jsNode) {
	...
	if(!doc || doc.parserError && doc.parseError.errorCode != 0 || !doc.documentElement) {
		if (!doc || doc.parseError.errorCode == 0) {
			...
			//jsNode.errorText = "加载失败 " + jsNode.src + " (" + jsNode._xmlHttp.status + ": " + jsNode._xmlHttp.statusText + ")";
			jsNode.errorText = "加载失败";
		} else {
			//jsNode.errorText = "Error loading " + jsNode.src + " (" + doc.parseError.reason + ")";
			//jsNode.errorText = "加载失败 " + jsNode.src + " (" + doc.parseError.reason + ")";
			jsNode.errorText = "加载失败";
		}
	} else {
	...
	if (jsNode.errorText != "") {
		...
		jsNode._loadingItem.toolTip = "点击重新读入";
