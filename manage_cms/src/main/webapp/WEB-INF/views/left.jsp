    	...
    	<c:forEach var="menu" items="${data.menuCatalogs }">
	        <div id="pre_${menu.id }">
	        	...
	        	<c:choose>
	        		...
	        		<c:otherwise>
		            <p><a class="lnd" id="${menu.id }" href="${ctxPath }${menu.linkUrl }${sep }manageCatalogId=${menu.id}" target="mainFrame"  title="${menu.description }"><img src="${ctxPath }/admin/images/${menu.smallImage }" border="0">${menu.name }</a></p>
	        		</c:otherwise>
	        	</c:choose>
				<%--如果有子节点 --%>
        		<c:set var="sep" value="&"></c:set>
	    		<c:if test="${not empty menu.children and menu.linkUrl eq '#'}">
		           ...
		           <c:forEach var="subMenu" items="${menu.children }">
		                <li class="snd"><a href="${ctxPath }${subMenu.linkUrl }${sep }manageCatalogId=${subMenu.id}" target="mainFrame" title="${subMenu.description}"><img src="${ctxPath }/admin/images/${subMenu.smallImage }" border="0">${subMenu.name}</a></li>
	    			</c:forEach>
		            </ul>
	    		</c:if>
	        </div>
