...
function BtnMouseOver() {
	if (event.srcElement.tagName.toUpperCase() != "IMG") return false;
...
function BtnMouseUp() {
	...
	// modify by 2009-10-31 解决IE8下无法上传图片的问题
...
function PopulateTB(y) {
	...
	for (i=0; i<elements.length; i++) {
		element = elements[i];
		if (element.tagName.toUpperCase() == "SCRIPT" || element.tagName == "!") continue;
...
function sizeChange(size){
	...
	// 全屏状态下不允许改变编辑区域大小
	if (sFullScreen=="1"){
		alert(lang["MsgNotChangeOnFullScreen"]);
		return false;
	}
