...
public class ArticleDaoImpl extends BaseDao implements ArticleDao
{
     ...
     */
    @Override
    public List<Object> findPublishArticleById(int catalogId, int rows)
    {
        try
        {
            List<Object> argList = new ArrayList<Object>();
            String sql = "SELECT ARTICLE_ID, TITLE, BRIEF, PUBLISH_DATE, URL FROM T_ARTICLE WHERE CATALOG_ID = ? AND TYPE = 0 AND STATE = 3 ORDER BY IS_HEAD,CREATE_DATE DESC";
            argList.add(new Integer(catalogId));
            return getJdbcTemplate().query(sql, argList.toArray(), rows);
        }
        catch (Exception ex)
        {
            logger.error("", ex);
