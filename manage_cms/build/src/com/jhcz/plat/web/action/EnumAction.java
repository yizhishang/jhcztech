...
import com.jhcz.plat.service.EnumTypeService;
...
public class EnumAction extends BaseAction
{
    ...
    * @描述：
    * @作者：袁永君
    * @时间：2011-3-26 下午08:52:07
    * @return
    */
    public String doDeleteType()
    {
        EnumTypeService EnumTypeService = (EnumTypeService) getService(EnumTypeService.class);
        EnumService service = (EnumService) getService(EnumService.class);
        //String[] idArray = getStrArrayParameter("id");
        String enumValue = RequestHelper.getString(getRequest(), "id");
        
        //删除类别下的所有数据字典
        service.delItemByType(enumValue, getSiteNo());
        addLog("删除数据字典", "删除[type=" + enumValue + "]下的所有数据字典");
        
        EnumTypeService.delItemByVal(enumValue, getSiteNo());
        addLog("删除数据字典类型", "删除数据字典类型[id=" + enumValue + "]");
        
        ScriptHelper.redirect(getResponse(), "enum.action?function=left");
        return NONE;
    }
